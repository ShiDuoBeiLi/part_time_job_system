package com.votre.controller;

import com.votre.entity.*;
import com.votre.service.*;
...
public class IndexController {
    ...
    public MsgResult registerAct(User user, @RequestParam("roleId") long roleId, HttpSession session) {
        ...
        int j = userService.searchUserByUserPhone(user.getPhone());

        if (i == 1) {
            return MsgResult.build(0, "用户名已存在");
        }
        if (j == 1) {
            return MsgResult.build(0, "该手机号已被注册");
        }else {
